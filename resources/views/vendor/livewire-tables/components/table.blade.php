...
@if ($isTailwind)
    <div
        ...
        {{ $attributes->merge($customAttributes['wrapper'])
            ->class([
                'bg-white rounded-2xl border border-gray-100 shadow-sm overflow-x-auto' => $customAttributes['wrapper']['default'] ?? true
            ])
        ...
        >
            <thead wire:key="{{ $tableName }}-thead"
                {{ $attributes->merge($customAttributes['thead'])
                    ->class([
                        'bg-gray-50' => $customAttributes['thead']['default'] ?? true
                    ])
                    ->except(['default','default-styling','default-colors']) }}
            >
                <tr>
            ...
            <tbody
                ...
                {{ $attributes->merge($customAttributes['tbody'])
                        ->class([
                            'bg-white divide-y divide-gray-100' => $customAttributes['tbody']['default'] ?? true
                        ])
                        ->except(['default','default-styling','default-colors']) }}
            >
                {{ $slot }}
